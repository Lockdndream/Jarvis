...
JNIEXPORT jint JNI_OnLoad(JavaVM* vm, void* /*reserved*/) {
    ...
    // Milestone 9B.6 Task 7: package renamed from the disposable spike's
    // com.jarvis.wakewordspike to production's com.jarvis.companion.wakeword
    // — this path must match exactly (JNI class lookup, not the JVM
    // classpath), or RegisterNatives below silently never runs and every
    // native method throws UnsatisfiedLinkError at first use.
    jclass clazz = env->FindClass("com/jarvis/companion/wakeword/MicroWakeWord");
    if (clazz == nullptr) {
